...
<head>
    ...
    <style>
        ...
        .backdoor-list li {
            ...
            /* Membungkus teks agar tidak melampaui lebar elemen */
        }
        ...
        /* Tambahkan gaya responsif berikut */
...
<body>
    <div class="container mt-5">
        <h1 class="text-center animate__animated animate__fadeInDown">Hasil Pemindaian</h1>
                    ...
                    <table class="table table-dark table-striped backdoor-table">
                        ...
                        <tbody>
                            <?php foreach ($backdoors as $backdoor): ?>
                                <tr>
                                    <?php
                                    $fileName = basename($backdoor['file']);
                                    $directory = dirname($backdoor['file']);
                                    ?>
                                    <td><?php echo $fileName; ?></td>
                                    <td><?php echo $directory; ?></td>
                                    <td><?php echo $backdoor['status']; ?></td>
                                    <td><?php echo $backdoor['score']; ?></td>
                                    <td><?php echo $backdoor['suspicion_level']; ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
                <a href="<?php echo site_url('scanner/export_backdoors'); ?>"
                    class="btn btn-secondary btn-custom mt-2">Ekspor ke .txt</a>
            </div>
        <?php endif; ?>
        <div class="terminal mt-4 animate__animated animate__fadeInUp scrollable">
            <?php foreach ($result as $res): ?>
                <p
                    class="text-<?php echo $res['color']; ?><?php echo (strpos($res['status'], 'Ditemukan') !== false) ? ' alert alert-warning' : ''; ?>">
                    <?php
                    $status = $res['status'];
                    if (strpos($status, 'Ditemukan') !== false) {
                        $status = str_replace('Ditemukan', '<span class="highlight">Ditemukan</span>', $status);
                    }
                    echo $res['file'] . ' => ' . $status;
                    if ($res['score'] > 0) {
                        echo ' | Skor: ' . $res['score'] . ' | Level Kecurigaan: ' . $res['suspicion_level'];
                    }
                    ?>
                </p>
                <hr>
        ...
        <div class="text-center mt-4">
            <a href="<?php echo site_url('scanner'); ?>"
                class="btn btn-secondary btn-custom animate__animated animate__pulse animate__infinite">Kembali</a>
        </div>
    </div>
    <footer class="bg-dark text-white text-center py-3">
        <div class="container">
            <p class="mb-0">&copy; <?= date('Y'); ?> Mode Gabut.</p>
            <p class="mb-0">Dibuat oleh <a href="https://koys.my.id" class="text-white">Koys</a></p>
            <div class="mt-2">
